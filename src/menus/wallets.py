...
async def handle_expectation_edit_wallet(session: AsyncSession,
                                         user: User, _, event):
    ...
    await session.execute(
        delete(WalletAlias).where(
            ...
            WalletAlias.alias == name
        )
    )

    wallet = await session.execute(
        select(Wallet).where(Wallet.id == uuid)
    )
    ...
    if wallet:
        ...
        await event.respond(_("wallet_edited_successfully").format(
            name, currency, init_sum
        ))
...
async def export(session: AsyncSession, event, user: User, _ ):
    ...
    writer.writerow(["created_at", "name", "currency", "init_sum",
                     "current_sum", "transaction_count", "is_deleted"])
...
async def handle_action(session: AsyncSession, event,
    ...
    if data[1][1] == "d":
        ...
        raise Exception(
            'Got unexpected data for callback command "action" (wallets)'
        )
...
async def edit_menu(session: AsyncSession, user: User, _, event,
    ...
    buttons = [
        Button.inline(_("wallet_action_edit_cancel"),
                      b"menu_wallets")
    ]
...
async def view_menu(session: AsyncSession, user: User, _, event,
    ...
    buttons = [
        Button.inline(_("universal_back_button"), b"menu_wallets")
    ]
    if len(transactions) == 0:
        await event.respond(_("wallet_action_view_no_transactions").format(
            wallet.icon, wallet.name, formatted_created_on,
            wallet.currency, wallet.init_sum
        ), buttons=buttons)
        return
    ...
    content = _("wallet_action_view").format(
        wallet.icon, wallet.name, formatted_created_on,
        wallet.currency, wallet.init_sum,
        wallet.transaction_count, transaction_component
    )

    # TODO: optimize to only select 5 latest
    full_aliases = await session.execute(
        select(WalletAlias)
        .where(WalletAlias.wallet == uuid)
    )
...
async def delete_menu(session: AsyncSession, user: User, _, event,
    ...
    buttons = [
        Button.inline(_("wallet_action_delete_approve"),
                      "action_wd_"+uuid.hex()),
        Button.inline(_("wallet_action_delete_cancel"),
                      b"menu_wallets")
    ]
    await event.respond(_("delete_wallet_prompt").format(name),
                        buttons=buttons)
...
async def send_menu(session: AsyncSession, user: User, _,
    ...
    if len(wallets) == 0:
        ...
        if del_wallets_count == 0:
            await event.respond(_("menu_wallets_no_wallets"),
                                buttons=buttons)
        else:
            await event.respond(_("menu_wallets_only_deleted").format(
                del_wallets_count
            ), buttons=buttons)
        return
    ...
    wallets = wallets[
        (page-1)*WALLETS_PER_PAGE:page*WALLETS_PER_PAGE
    ]
    ...
    content = _("menu_wallets_template").format(
        wallet_info_str, wallets_count)
    if del_wallets_count != 0:
        content += _("menu_wallets_component_deleted_amount").format(
                del_wallets_count
            )
